                        ...
                        if current_dir.join(tag).is_dir() {
                            ...
                            for sub_tag in tags.iter() {
                                ...
                                if current_dir.join(tag).join(sub_tag).is_dir() && sub_tag != tag {
                                    ...
                                    let _ = std::fs::copy(
                                        &yaml_path,
                                        current_dir.join(tag).join(sub_tag).join(
                                            yaml_path
                                                .file_name()
                                                .unwrap()
                                                .to_string_lossy()
                            ...
                            if !has_product && tags.len() == 1 {
                                ...
                                let _ =
                                    std::fs::create_dir_all(current_dir.join(tag).join(product));
                                let _ = std::fs::copy(
                                    &yaml_path,
                                    current_dir.join(tag).join(product).join(
                                        yaml_path
                                            .file_name()
                                            .unwrap()
                                            .to_string_lossy()
                ...
                Ok(template) => {
                    ...
                    if let Some(vpf) = vpf {
                        ...
                        if p.exists() && p.is_dir() {
                            std::fs::create_dir_all(current_fingerprint_dir.join(&vpf.vendor))
                                .unwrap();
