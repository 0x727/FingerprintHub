...
def create_fingerprint(name):
    ...
    base_path = Path("web-fingerprint").resolve()
    n = Path("web-fingerprint/00_unknown/").joinpath(name).with_suffix(".yaml").resolve()
...
def run_observer_ward(name):
    ...
    #     observer_ward = "/home/kali-team/observer_ward"
    proc = subprocess.Popen(args=[observer_ward, "-t", target,
                                  "-p", n, "--debug"], shell=False, stdout=subprocess.PIPE,
